...
def main():
    global current_hwnd
    print("="*50);print("Meitu Sticker v3.1");print("Target X=%d Y=%d"%(TARGET_X,TARGET_Y));print("="*50)
    up=get_unpro()
    ...
    for i,dn in enumerate(up):
        ...
        if i==0:
            ...
            print("  [PREVIEW] 已移动到位，按要求在保存前停止；未混合、未保存、未标记 processed。")
            print("  [PREVIEW] 请直接查看美图窗口里的贴图位置。")
            return
        else:
            if hwnd and lp:
                ff(hwnd)
