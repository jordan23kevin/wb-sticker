...
4. **keep_alive切胚衣**：不杀进程，Ctrl+W关闭→Popen新胚衣→等新窗口句柄→定位→enter_ai_sticker
5. **窗口标题**：繁简体兼容(美圖秀秀/美图秀秀)，find_meitu只认"美圖秀秀-图片编辑"
6. **文件对话框**：QT应用，找#32770类名弹窗
7. **保存文件名去扩展名**：对话框自动加.jpg
8. **黑T审查**：跑完同步到01_CHECK给用户人工审查
9. **保存失败停下**：save_image返回False时pipeline立即停，不跳过下一张
10. **默认名改名**：美图保存为"白背2_副本.jpg"/"白正2_副本.jpg"，需改名成"DXxxxx_B_白T.jpg"/"DXxxxx_W_白T.jpg"
11. **switch_torso后等1.5s**：T恤加载完再进AI贴图，否则add_image点不到
12. **手动模式**：用户可手动切去背模式+手动保存，脚本检测用时超平均3s判手动，等5s后全局扫描归位
